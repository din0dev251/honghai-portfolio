    ...
    >
      <nav className="container mx-auto max-w-7xl px-4 mobile:px-6 md:px-8 lg:px-12">
        <div className="flex items-center justify-between h-16 mobile:h-20">
          ...
          <div className="hidden lg:flex items-center gap-6 justify-end w-full">
            {navLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => scrollToSection(link.href)}
                className={`text-sm font-heading font-medium hover:text-primary transition-colors ${isScrolled ? "text-black" : "text-white"}`}
              >
                {t(link.key as "home")}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-base font-heading font-medium text-foreground hover:text-primary transition-colors"
            >
              <Image
                src={
                  locale === "en"
            ...
            <button
              className={cn(
                "p-2 rounded-full transition-transform duration-200 ease-out focus:outline-none focus:ring-2 focus:ring-primary/40",
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden pb-4 space-y-2 bg-background/95 backdrop-blur-md rounded-b-2xl animate-mobile-menu-down origin-top">
            {navLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => scrollToSection(link.href)}
